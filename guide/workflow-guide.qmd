...
Now that you're set up, here is what the system actually *does*. This section shows the three core mechanisms that make the workflow powerful: specialized agents, adversarial QA, and automatic quality scoring.
...
Now that you're up and running, here is how the system is configured under the hood. Claude Code's power comes from five configuration layers that work together. Think of them as the operating system for your academic project. Understanding these layers helps you customize the workflow and debug when things go wrong.
